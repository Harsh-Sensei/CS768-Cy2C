...
class Trainer(object):
    ...
    def __init__(self, model_name, dataset_name,dataset,device,MODEL_CLASS,num_node_features,num_classes,batch_size=128,hidden_dim=138,lr=7e-4,n_layer=3,num_workers=4,opt_fun=1,small_fold=10,drop_mid=0.0,drop_ini=0.0,decay=0.0, main_fold=10, Cy2C=True, base=False):
        ...
        self.small_fold=small_fold
        self.drop_mid=drop_mid
        self.drop_ini=drop_ini
        self.main_fold=main_fold
        self.decay=decay
         #전체에서 1번만
    ...
    def load_model(self):
        model=self.MODEL_CLASS(self.hidden_dim,self.num_node_features,self.num_classes,n_layer=self.n_layer,drop_mid=self.drop_mid,drop_ini=self.drop_ini).to(self.device)  
        return model
    ...
    def train(self):                              
        ...
        for mainfold_idx in range(self.main_fold):
                       
                              
            for subfold_idx in range(self.small_fold):
                self.epoch_results = {
                    'val_loss': [],
                    'val_acc': [],
                    'train_loss': [],
...
#             os.makedirs(f'{self.check_path}/{self.model_name}')
        ...
        if self.Cy2C == True:
            ...
            if self.base == True :
                f_txt = open(f'{self.result_path}/cy2c_base_model_comparision.txt', 'a')
        else : 
            f_txt = open(f'{self.result_path}/(base)model_comparision.txt', 'a')
...
class Trainer_fix(object):
    
    def __init__(self, model_name, dataset_name,dataset,device,MODEL_CLASS,num_node_features,num_classes,batch_size=128,hidden_dim=138,lr=7e-4,n_layer=3,num_workers=4,opt_fun=1):
        super(Trainer_fix, self).__init__()        
        self.num_workers=num_workers
        self.lr=lr
        self.opt_fun=opt_fun
        self.n_layer=n_layer
        ...
        if os.path.isdir(self.check_path) & os.path.isdir(self.result_path) :
            print('folder_on')
        else:
            os.makedirs(self.check_path)
            os.makedirs(self.result_path)  
    ...
    def load_model(self):
        ...
        model=self.MODEL_CLASS(self.hidden_dim,self.num_node_features,self.num_classes,n_layer=self.n_layer).to(self.device)  
        return model
    ...
    def train(self):                              
        ...
        for mainfold_idx in range(1):
                       
                              
            for subfold_idx in range(1):
                self.epoch_results = {
                    'val_loss': [],
                    'val_acc': [],
                    'train_loss': [],
...
#             f_txt.close()        
    ...
    def test(self, loader):
        ...
        with torch.no_grad():
            for data in loader:  # Iterate in batches over the training/test dataset.
                #print(len(data))
                data=data.to(self.device)
                out = self.model(data.xx, data.edge_index, data.cycle_index, data.batch)  
                pred = out.argmax(dim=1)  # Use the class with highest probability.
                correct += int((pred == data.y).sum())  # Check against ground-truth labels.
                loss = self.criterion(out, data.y)
                total_loss+=loss.item()
    ...
    def train_train(self):          
        ...
        for data in self.train_loader:  # Iterate in batches over the training dataset.
            self.optimizer.zero_grad()
            data=data.to(self.device)
            out = self.model(data.xx, data.edge_index, data.cycle_index, data.batch)  # Perform a single forward pass.
            loss =self.criterion(out, data.y)  # Compute the loss.
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), 1.0)
            loss.backward()  # Derive gradients.
            self.optimizer.step()  # Update parameters based on gradients.
...
class Trainer_fix_attr(object):
    
    def __init__(self, model_name, dataset_name,dataset,device,MODEL_CLASS,num_node_features,num_classes,batch_size=128,hidden_dim=138,lr=7e-4,n_layer=3,num_workers=4,opt_fun=1):
        super(Trainer_fix_attr, self).__init__()        
        self.num_workers=num_workers
        self.lr=lr
        self.opt_fun=opt_fun
        self.n_layer=n_layer
...
#                 }                      
                ...
                if self.opt_fun ==1:
                    ...
                    self.optimizer = torch.optim.RAdam(self.model.parameters(), lr=self.lr)
                self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min',factor=0.5, patience=25, min_lr=minimum_lr)
                self.criterion = nn.CrossEntropyLoss(reduction='sum')
...
#             f_txt.close()        
    ...
    def test(self, loader):
        ...
        with torch.no_grad():
            for data in loader:  # Iterate in batches over the training/test dataset.
                #print(len(data))
                data=data.to(self.device)
                out = self.model(data.xx, data.edge_index, data.cycle_index, data.edge_attr, data.batch)  
                pred = out.argmax(dim=1)  # Use the class with highest probability.
                correct += int((pred == data.y).sum())  # Check against ground-truth labels.
                loss = self.criterion(out, data.y)
                total_loss+=loss.item()
    ...
    def train_train(self):          
        ...
        for data in self.train_loader:  # Iterate in batches over the training dataset.
            self.optimizer.zero_grad()
            data=data.to(self.device)
            out = self.model(data.xx, data.edge_index, data.cycle_index, data.edge_attr, data.batch)  # Perform a single forward pass.
            loss =self.criterion(out, data.y)  # Compute the loss.
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), 1.0)
            loss.backward()  # Derive gradients.
            self.optimizer.step()  # Update parameters based on gradients.
